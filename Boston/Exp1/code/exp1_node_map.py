...
def main():
    ...
    for _, _, _, data in G.edges(keys=True, data=True):
        data["weight"] = float(data.get("length", 1.0))
    ...
    fig, ax = ox.plot_graph(
        G,
        node_size=5,          # muestra nodos
        node_color="white",   # nodos visibles sobre fondo blanco
    ...
    fig.savefig("exp1_lambda1_cut_with_nodes.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
